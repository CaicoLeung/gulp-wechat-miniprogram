...
// @ts-ignore
const jsonUtil = require('./gulp/json');
...
const sassFiles = [
  `${appPath}/*.+(sass|scss)`,
  `!${appPath}/assets/css/*.+(sass|scss)`,
  `!${appPath}/_template/*.+(sass|scss)`
];
const jsonFiles = [`${appPath}/*.json`, `!${appPath}/_template/*json`];
const tsFiles = [
  `${appPath}/*.ts`,
  `!${appPath}/_template/*ts`,
  `!${appPath}/*.d.ts`
];
...
const typescript = () => {
  return gulp
    .src(tsFiles, { since: gulp.lastRun(typescript) })
...
const sass = () => {
  ...
  return gulp
    ...
    .pipe(sourcemaps.init())
    .pipe(
      sassParse({ outputStyle: 'compressed' }).on('error', sassParse.logError)
    )
...
const images = () => {
  return gulp
    .src(imgFiles, { since: gulp.lastRun(images) })
    .pipe(imagemin())
    .pipe(gulp.dest(distPath));
};
gulp.task(images);
...
gulp.task(
  ...
  gulp.series(
    'clean',
    gulp.parallel('wxml', 'typescript', 'json', 'sass', 'images')
  )
);
...
const argv = ('argv' in yargs) && yargs['argv'];
let target = 'test';
let appJson = jsonUtil.readAppJson();

const create = () => {
  if (argv['page']) target = argv['page'];
  if (appJson.pages) {
    appJson.pages.push(`pages/${target}/${target}`);
    jsonUtil.writeAppJson(JSON.stringify(appJson));
  }
  return gulp
